 ...
 */
export function documentToHtmlStringsAndAstroComponents(
  richTextDocument: Document,
  options: Partial<Options> = {}
): (string | AstroComponent)[] {
  if (!richTextDocument || !richTextDocument.content) {
    return [''];
  }

  return nodeListToHtmlStringAndAstroComponents(richTextDocument.content, {
    renderNode: {
...
function nodeListToHtmlStringAndAstroComponents(
  nodes: CommonNode[],
  { renderNode, renderMark }: Options
): (string | AstroComponent)[] {
  const computed = nodes.map<string | AstroComponent>((node) =>
    nodeToHtmlStringAndAstroComponent(node, { renderNode, renderMark })
  );
  const result: (string | AstroComponent)[] = [''];
  computed.forEach((val) => { 
    if (typeof val === 'string') {
      // We have a string here, we want to concatenate this chunk with the last string in the array
...
function nodeToHtmlStringAndAstroComponent(
  node: CommonNode,
  { renderNode, renderMark }: Options
): string | AstroComponent {
  if (helpers.isText(node)) {
    ...
    const nextNode: Next = (nodes) =>
      nodeListToHtmlStringAndAstroComponents(nodes, { renderMark, renderNode });
